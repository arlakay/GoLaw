 ...
 */
public class AdvokatAdapter extends RecyclerView.Adapter<AdvokatAdapter.VersionViewHolder> {

    private List<Advokat> loginList;
    ...
    public AdvokatAdapter(List<Advokat> login, int rowLayout, Context context, OnItemClickListener listener) {
    ...
    public void onBindViewHolder(VersionViewHolder versionViewHolder, int i) {
        final Advokat model = loginList.get(i);
    ...
    public void animateTo(List<Advokat> models) {
        applyAndAnimateRemovals(models);
        applyAndAnimateAdditions(models);
        applyAndAnimateMovedItems(models);
    }

    private void applyAndAnimateRemovals(List<Advokat> newModels) {
    ...
    private void applyAndAnimateAdditions(List<Advokat> newModels) {
        for (int i = 0, count = newModels.size(); i < count; i++) {
            final Advokat model = newModels.get(i);
    ...
    private void applyAndAnimateMovedItems(List<Advokat> newModels) {
        for (int toPosition = newModels.size() - 1; toPosition >= 0; toPosition--) {
            final Advokat model = newModels.get(toPosition);
    ...
    public Advokat removeItem(int position) {
        final Advokat model = loginList.remove(position);
        notifyItemRemoved(position);
        return model;
    }

    public void addItem(int position, Advokat model) {
        loginList.add(position, model);
        notifyItemInserted(position);
    }

    public void moveItem(int fromPosition, int toPosition) {
        final Advokat model = loginList.remove(fromPosition);
    ...
    class VersionViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(final Advokat model, final OnItemClickListener listener) {
            titleRes.setText(model.getName());
            descRes.setText(model.getDescription());
